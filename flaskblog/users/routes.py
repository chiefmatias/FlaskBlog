...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Your account was created! You are now able to login in.', 
              'success')
...
@users.route("/reset_password/<signature>", methods=['GET', 'POST'])
def reset_token(signature):
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))
    
    protected = request.args.get('protected')
    payload = request.args.get('payload')
    token = {'protected': protected, 'signature': signature , 'payload': payload}
